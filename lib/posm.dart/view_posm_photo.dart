...
class ViewPosmPhotos extends StatefulWidget {
  static const routeName = "view-Posm-Photos";
  const ViewPosmPhotos({super.key});

  @override
  State<ViewPosmPhotos> createState() => _ViewPosmPhotosState();
}

class _ViewPosmPhotosState extends State<ViewPosmPhotos> {
  bool isInit = true;
  int activeImage = 0;
  ...
  void didChangeDependencies() async {
    if (isInit) {
      ...
      try {
        ...
        await Provider.of<PosmProvider>(context)
            .fetchPosmPhotos(
                ...
                routeArgs["reportRecordId"].toString())
            .then((value) {
          if (value) {
            if (mounted) {
              setState(() {
  ...
  Future deleteImageConfirmation(int imageIndex, int imageId) {
    return showDialog(
      context: context,
  ...
  void deleteImage(int imageIndex, int imageId) async {
    if (imageIndex > 0) {
    ...
    try {
      ...
      await Provider.of<PosmProvider>(context, listen: false)
          .deletePosmCapturePhoto(
              routeArgs["posmMapKey"],
              routeArgs["reportRecordId"],
              imageIndex,
              routeArgs["userName"].toString(),
              routeArgs["workingId"].toString(),
              routeArgs["storeId"].toString(),
        ...
        Provider.of<PosmProvider>(context, listen: false)
            .decrementPhotoNo(routeArgs["posmIndex"]);
      });
    } catch (error) {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final imageData =
        Provider.of<PosmProvider>(context, listen: false).getPosmPhotoData;
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Container(
                                  child: IconButton(
                                    onPressed: () {
                                      deleteImageConfirmation(activeImage,
                                          imageData[activeImage].id);
                                    },
                                    icon: Icon(
                                      Icons.delete,
                                      color: Colors.red,
